...
def collab_model(movie_list,top_n=10):
    ...
    new_row3 = {'userId':200000, 'movieId':movie_ids[2], 'rating': 5.0}
    df_init_users = df_init_users.append([new_row1,new_row2,new_row3],ignore_index=True)

    util_matrix = pd.pivot_table(df_init_users,values='rating',columns='userId',index='movieId')
    util_matrix.fillna(0, inplace=True)
    
    cosine_sim=cosine_similarity(np.array(util_matrix),np.array(util_matrix))
    
    ls = list(util_matrix.index)
    df = movies_df[movies_df['movieId'].isin(ls)].reset_index(drop=True)
